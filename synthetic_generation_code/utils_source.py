# -*- coding: utf-8 -*-
"""
Created on Fri Jul 15 16:21:32 2022
...
__all__ = ['PointSource', 'Receiver', 'Shot', 'WaveletSource',
           'RickerSource','RickerSource_MinimumPhase', 'GaborSource', 'DGaussSource', 'TimeAxis']
...
class RickerSource_MinimumPhase(WaveletSource):
    ...
    def wavelet(self):
        print(self.f0*100)
        w, t = bg.filters.ricker(duration=self.time_values[-1]/1000, dt=self.time_values[1]/1000-self.time_values[0]/1000, f=self.f0*1000)
        minimum_phase_ricker=minimum_phase(np.squeeze(w))
        minimum_phase_ricker=np.pad(minimum_phase_ricker,(0,t.shape[0]-minimum_phase_ricker.shape[0]), 'constant', constant_values=(0,0))
        print(minimum_phase_ricker.shape)
        return minimum_phase_ricker

class Ormsby_MinimumPhase(WaveletSource):
    ...
    def wavelet(self):
        print(self.f0)
        duration=self.time_values[-1]/1000 
        dt=self.time_values[1]/1000-self.time_values[0]/1000 
        print(dt)
        f=self.f0
        w, t = bg.filters.ormsby(duration=duration, dt=dt, f=f)
        
        minimum_phase_ormsby=minimum_phase(np.squeeze(w))
        minimum_phase_ormsby=np.pad(minimum_phase_ormsby,(0,t.shape[0]-minimum_phase_ormsby.shape[0]), 'constant', constant_values=(0,0))
        print(minimum_phase_ormsby.shape)
        return minimum_phase_ormsby
